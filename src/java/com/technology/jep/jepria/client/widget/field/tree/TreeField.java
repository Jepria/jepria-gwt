...
public class TreeField<V extends JepOption> extends Composite implements HasCheckChangeHandlers<V> {
   ...
   */
  private Map<Object, TreeNodeInfo<V>> nodeMapOfDisplay = new HashMap<>();
  ...
  protected CellTree getTree() {
    ...
    result.addOpenHandler(new OpenHandler<TreeNode>() {
      ...
      public void onOpen(OpenEvent<TreeNode> event) {
        ...
        TreeNodeInfo<V> info = nodeMapOfDisplay.get(currentNode != null ? currentNode.getValue() : null);
  ...
  public boolean isNodeOpened(TreeNode node, V value){
    for(int i = 0; i < node.getChildCount(); i++){
      ...
      if(value.getValue().equals(entityChild.getValue())){
   ...
   */
  public boolean setChecked(V value, boolean checked){
    if (value != null) {
      TreeNodeInfo<V> nodeInfo = nodeMapOfDisplay.get(value.getValue());
      if (nodeInfo != null) {
        selectionModel.setSelected(nodeInfo.getValue(), checked);
        return true;
      }
    }
    return false;
  ...
  public TreeNodeInfo<V> getNodeInfoByValue(V node){
    return nodeMapOfDisplay.get(node != null ? node.getValue() : null);
  }
  ...
  protected SelectAction translateSelectionEvent(CellPreviewEvent<V> event) {
    ...
    if (CLICK.equals(nativeEvent.getType())) {
      ...
      if (!availableSelectedNodes.contains(currentNode)) return SelectAction.IGNORE;
  ...
  class TreeDataProvider extends AsyncDataProvider<V> {
        ...
        loader.load(expandNode, new JepAsyncCallback<List<V>>() {
          ...
          public void onSuccess(List<V> result) {
            ...
            for (V value : result){
              ...
              if (nodeMapOfDisplay.containsKey(value != null ? value.getValue() : null)) {
                nodeMapOfDisplay.get(value != null ? value.getValue() : null).setDisplay(display);
              } else {
                nodeMapOfDisplay.put(value.getValue(), new TreeNodeInfo<V>(display, result, null, expandNode, value));
              }
     ...
     */
    public void refreshNode(V value){
      TreeNodeInfo<V> nodeInfo = nodeMapOfDisplay.get(value != null ? value.getValue() : null);
  ...
  class TreeFieldSelectionModel extends MultiSelectionModel<V> implements RefreshStartEvent.Handler, RefreshEndEvent.Handler {
    ...
    public TreeFieldSelectionModel() {
      super();
      super.addSelectionChangeHandler(event -> {
        ...
        // Вынос вычислений в EventLoop, чтобы не тормозить рендер страницы
    ...
    public void selectChildren(V item, boolean selected) {
      boolean isPartialCheckStyle = checkStyle.equals(CheckCascade.PARENTS) || checkStyle.equals(CheckCascade.BOTH);
      List<V> children = getChildrenNodes(item);      
      TreeNodeInfo<V> treeNodeInfo = getNodeInfoByValue(item);
      if (children != null){
        for (V child : children){
          if (isPartialCheckStyle && partialSelectedNodes.contains(child)) {
